...
import com.mcgrady.xproject.pokemon.databinding.PokemonItemDbBinding
import com.mcgrady.xproject.pokemon.ui.databinding.main.PokemonListDBActivity

/**
 * Created by mcgrady on 2022/1/7.
 */
class PokemonDBAdapter :
    RecyclerView.Adapter<PokemonDBAdapter.PokemonViewHolder>(),
...
//        val view = parent.inflate(R.layout.item_pokemon)
        return PokemonViewHolder(BindingViewHolder(parent, PokemonItemDbBinding::inflate).binding).apply {
...
//            })
            binding.root.setOnClickListener { v ->
                ...
                val intent = Intent(v?.context, PokemonListDBActivity::class.java).apply {
                    putExtra("EXTRA_POKEMON", items[position])
                }
                TransformationCompat.startActivity(binding.transformationLayout, intent)
            }
    ...
    class PokemonViewHolder(val binding: PokemonItemDbBinding) : RecyclerView.ViewHolder(binding.root) {
